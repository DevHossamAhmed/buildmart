...
export interface UsersTeamsFilters {
  search: string;
  status: string;
  department: string;
  role: string;
...
interface UsersTeamsFilterBarProps {
  filters: UsersTeamsFilters;
  onFiltersChange: (filters: UsersTeamsFilters) => void;
  onExport?: () => void;
...
const UsersTeamsFilterBar: React.FC<UsersTeamsFilterBarProps> = ({
  filters,
  onFiltersChange,
  onExport,
  statusOptions,
  ...
  const handleFilterChange = (key: keyof UsersTeamsFilters, value: string) => {
    onFiltersChange({
      ...filters,
      [key]: value,
    });
  ...
  return (
    <div className={`bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-5 ${className}`}>
      <div className="flex flex-col gap-4">
        ...
        {/* Expandable Filters Section - Only for Users */}
...
export default UsersTeamsFilterBar;
